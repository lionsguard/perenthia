...
namespace Radiance.Providers
{
	internal static class ProviderUtil
	{
		...
		public static TProvider InstantiateProviders<TProvider, TProviderCollection>(
			ProviderSettingsCollection configProviders, 
			string defaultProvider, 
			TProviderCollection providers)
			where TProvider : ProviderBase
			where TProviderCollection : ProviderCollection
		{
			ProvidersHelper.InstantiateProviders(configProviders, providers, typeof(TProvider));
			return providers[defaultProvider] as TProvider;
		}
	}
}
